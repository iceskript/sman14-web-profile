  ...
  return (
    <section className="py-24 bg-white font-urbanist overflow-hidden">
      <div className="max-w-[1440px] mx-auto px-5 lg:px-[100px]">
        ...
        {/* --- HEADER SECTION --- */}
        <header className="flex flex-col items-start mb-20">
          ...
          >
            {alumniData.map((item) => (
              <SwiperSlide key={item._id} className="h-auto pb-10">
                ...
                */}
                <article className="relative bg-white rounded-[15px] p-10 lg:p-14 shadow-[0_30px_60px_rgba(0,0,0,0.12)] drop-shadow-[0_10px_10px_rgba(0,0,0,0.05)] border border-gray-100 flex flex-col h-full ml-10 mr-4 transition-all duration-300 hover:shadow-[0_40px_80px_rgba(0,0,0,0.15)]">
                  
                  {/* FOTO ALUMNI */}
                  <figure className="absolute -top-12 -left-10 w-24 h-24 lg:w-32 lg:h-32 bg-[#D9D9D9] rounded-full border-[8px] border-white shadow-xl z-30 overflow-hidden">
                    {item.foto ? (
                      <img 
                  ...
                  {/* IDENTITAS */}
                  <header className="ml-14 lg:ml-20 mb-8">
                  ...
                  <blockquote className="relative flex-grow">
                    <Quote className="absolute -top-3 -left-7 text-[#1A1A1A] w-6 h-6 fill-current opacity-20" aria-hidden="true" />
